...
import { beliefs, cavs, events, messages, ministries, site, testimonials } from '../lib/data';
...
import {
  btnGhost, btnPrimary, btnPrimarySm, card, display, kicker, sectionTitle, wrap,
} from '../lib/ui';

export default function Home() {
  useReveal();
  const countdown = useCountdown();
  const { copied, copy } = usePixCopy(site.pixCnpj);
  const { openHost, openReg } = useModal();
      ...
      <header style={{ position: 'relative', minHeight: '100vh', display: 'flex', flexDirection: 'column', justifyContent: 'flex-end', overflow: 'hidden', padding: '0 0 64px' }}>
        ...
        <div style={{ position: 'relative', zIndex: 3, ...wrap, padding: '0 28px', width: '100%' }}>
          ...
          <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap', alignItems: 'center', animation: 'rise 1s var(--ease) .3s both' }}>
            <Link to="/quem-somos" className="btn-glow" style={btnPrimary}>Quero visitar →</Link>
            <Link to="/aovivo" className="btn-ghost-pulse" style={btnGhost}>
      ...
      <div style={{ borderTop: '1px solid var(--border-soft)', borderBottom: '1px solid var(--border-soft)', background: 'var(--ink)' }}>
        <div style={{ ...wrap, padding: '22px 28px', display: 'flex', flexWrap: 'wrap', gap: '18px 40px', alignItems: 'center', justifyContent: 'space-between' }}>
          <div style={{ display: 'flex', flexWrap: 'wrap', gap: '14px 36px' }}>
            {[
              ['Domingo', '10h · Manhã  |  18h · Família'],
              ['Quarta-feira', '20h · Culto de Ensino'],
              ['Onde', 'R. Mal. Rondon, 163 · Amambai'],
            ].map(([k, v]) => (
          ...
          <Link to="/contato" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.08em', color: 'var(--glow)', cursor: 'pointer', whiteSpace: 'nowrap', textDecoration: 'none' }}>Como chegar →</Link>
        </div>
      </div>
      ...
      <section className="reveal" style={{ padding: '0 28px 108px' }}>
        <div data-split style={{ ...wrap, display: 'grid', gridTemplateColumns: '1.05fr .95fr', gap: 54, alignItems: 'center' }}>
          ...
          <div>
            ...
            <Link to="/quem-somos" className="btn-ghost" style={{ ...btnGhost, padding: '14px 26px' }}>Conheça nossa história →</Link>
          </div>
        </div>
      </section>
      ...
      <section className="reveal" style={{ padding: '104px 28px', borderTop: '1px solid var(--border-soft)', background: 'linear-gradient(180deg,var(--ink),var(--void))' }}>
        <div style={wrap}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 20, marginBottom: 46 }}>
            ...
            <Link to="/ministerios" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.08em', color: 'var(--glow)', whiteSpace: 'nowrap', textDecoration: 'none' }}>Ver todos →</Link>
          </div>
          <div data-grid-min style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 16 }}>
            {ministries.map((m) => (
              <div key={m.id} className="lift zoom-parent" onClick={() => navigate(`/ministerios/${m.id}`)} style={{ position: 'relative', borderRadius: 16, overflow: 'hidden', cursor: 'pointer', aspectRatio: '3/4', border: '1px solid var(--border)' }}>
        ...
        <div data-split style={{ ...wrap, display: 'grid', gridTemplateColumns: '1fr .8fr', gap: 48, alignItems: 'center' }}>
          <div>
            ...
            <div style={{ display: 'flex', gap: 14, flexWrap: 'wrap' }}>
              <Link to="/cav" className="btn-glow" style={btnPrimarySm}>Encontrar uma CAV</Link>
              <button onClick={openHost} className="btn-ghost" style={{ ...btnGhost, padding: '13px 24px' }}>Quero ser anfitrião</button>
            </div>
          </div>
          <div style={{ display: 'flex', flexDirection: 'column', gap: 14 }}>
            ...
            <Link to="/cav" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.08em', color: 'var(--glow)', paddingTop: 4, textDecoration: 'none' }}>Ver todas as casas →</Link>
          </div>
        </div>
      </section>
          ...
          <div data-split style={{ position: 'relative', display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: 44, alignItems: 'center' }}>
            <div>
              ...
              <div style={{ display: 'flex', gap: 12, flexWrap: 'wrap' }}>
                <Link to="/aovivo" className="btn-pulse" style={{ ...btnPrimarySm, background: 'var(--pulse)', color: '#fff' }}>
                  <span style={{ width: 8, height: 8, borderRadius: '50%', background: '#fff', animation: 'pulse 1.4s infinite' }} />Assistir agora
                </Link>
                <Link to="/programacao" className="btn-ghost" style={{ ...btnGhost, padding: '13px 24px' }}>Ver programação</Link>
              </div>
            </div>
      ...
      <section className="reveal" style={{ padding: '104px 28px', borderTop: '1px solid var(--border-soft)', background: 'linear-gradient(180deg,var(--ink),var(--void))' }}>
        <div style={wrap}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 20, marginBottom: 46 }}>
            ...
            <Link to="/eventos" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.08em', color: 'var(--glow)', whiteSpace: 'nowrap', textDecoration: 'none' }}>Agenda completa →</Link>
          </div>
            ...
            {events.map((e) => (
              <div key={e.id} className="lift" style={{ position: 'relative', borderRadius: 18, overflow: 'hidden', border: '1px solid var(--border)', minHeight: 300, display: 'flex', flexDirection: 'column', justifyContent: 'flex-end' }}>
                ...
                <div style={{ position: 'relative', padding: 22 }}>
                  ...
                  <button onClick={() => openReg(e)} className="btn-fill-glow" style={{ display: 'inline-flex', alignItems: 'center', gap: 8, background: 'rgba(255,255,255,.08)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--head)', fontWeight: 600, fontSize: 13.5, padding: '10px 20px', borderRadius: 99, cursor: 'pointer', backdropFilter: 'blur(6px)' }}>Fazer inscrição →</button>
                </div>
              </div>
            ))}
          </div>
      ...
      <section className="reveal" style={{ padding: '104px 28px', borderTop: '1px solid var(--border-soft)' }}>
        <div style={wrap}>
          <div style={{ display: 'flex', justifyContent: 'space-between', alignItems: 'flex-end', flexWrap: 'wrap', gap: 20, marginBottom: 46 }}>
            ...
            <Link to="/mensagens" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.08em', color: 'var(--glow)', whiteSpace: 'nowrap', textDecoration: 'none' }}>Biblioteca completa →</Link>
          </div>
          <div data-grid-msg style={{ display: 'grid', gridTemplateColumns: 'repeat(3,1fr)', gap: 16 }}>
            {messages.slice(0, 3).map((msg) => (
              <div key={msg.id} className="lift-sm" onClick={() => navigate('/mensagens')} style={{ cursor: 'pointer', borderRadius: 16, overflow: 'hidden', border: '1px solid var(--border)', background: 'var(--s1)' }}>
        ...
        <div style={{ ...wrap, border: '1px solid var(--border)', borderRadius: 24, overflow: 'hidden', position: 'relative' }}>
          ...
          <div data-split style={{ position: 'relative', padding: '54px 46px', display: 'grid', gridTemplateColumns: '1fr .9fr', gap: 44, alignItems: 'center' }}>
            ...
            <div style={{ background: 'rgba(11,11,12,.7)', backdropFilter: 'blur(10px)', border: '1px solid var(--border)', borderRadius: 18, padding: 26 }}>
              ...
              <div onClick={copy} className="hov-glow" style={{ display: 'flex', alignItems: 'center', justifyContent: 'space-between', gap: 12, background: 'var(--void)', border: '1px solid var(--border)', borderRadius: 12, padding: '14px 16px', cursor: 'pointer', marginBottom: 14 }}>
                <span style={{ fontFamily: 'var(--mono)', fontSize: 14, color: 'var(--text)', letterSpacing: '.02em' }}>{site.pixCnpj}</span>
                <span style={{ fontFamily: 'var(--head)', fontWeight: 700, fontSize: 12, color: 'var(--glow)', whiteSpace: 'nowrap' }}>{copied ? '✓ Copiado' : 'Copiar'}</span>
              </div>
              <Link to="/contribua-pagamento" className="btn-glow" style={{ display: 'flex', alignItems: 'center', justifyContent: 'center', gap: 9, background: 'var(--glow)', color: '#050505', fontFamily: 'var(--head)', fontWeight: 700, fontSize: 15, padding: 15, borderRadius: 12, cursor: 'pointer', textDecoration: 'none' }}>Contribuir com cartão →</Link>
            </div>
          </div>
        </div>
      </section>
          ...
          <div style={{ textAlign: 'center', padding: '60px 0 56px', position: 'relative' }}>
            ...
            <div style={{ position: 'relative' }}>
              ...
              <div style={{ display: 'flex', gap: 14, justifyContent: 'center', flexWrap: 'wrap' }}>
                <Link to="/quem-somos" className="btn-glow" style={btnPrimary}>Planejar minha visita →</Link>
                <Link to="/aovivo" className="btn-ghost" style={btnGhost}>Assistir ao vivo</Link>
              </div>
            </div>
          </div>
          <div data-split style={{ display: 'grid', gridTemplateColumns: '1.1fr .9fr', gap: 0, border: '1px solid var(--border)', borderRadius: 22, overflow: 'hidden' }}>
            <div style={{ padding: '38px 36px', background: 'var(--s1)' }}>
              ...
              <div style={{ display: 'flex', flexDirection: 'column', gap: 14, marginBottom: 24 }}>
                {[
                  ['DOMINGO', '10h · Culto da Manhã  ·  18h · Culto da Família'],
                  ['QUARTA', '20h · Culto de Ensino'],
                  ['CEP', site.cep],
                ].map(([k, v]) => (
...
export function CountdownCard({ countdown, dark }) {
  ...
  return (
    <div style={{ background: dark ? 'var(--s1)' : 'var(--void)', border: '1px solid var(--border)', borderRadius: 18, padding: 26 }}>
      ...
      <div style={{ fontFamily: 'var(--head)', fontWeight: 700, fontSize: 20, marginBottom: 20 }}>{countdown.label}<span style={{ color: 'var(--glow)' }}> · {countdown.date}</span></div>
      <div style={{ display: 'grid', gridTemplateColumns: 'repeat(4,1fr)', gap: 10 }}>
        {[['Dias', countdown.days], ['Horas', countdown.hours], ['Min', countdown.mins], ['Seg', countdown.secs]].map(([lbl, val]) => (
