...
import { display, field, fieldLabel, wrap } from '@/lib/site-ui';
...
export default function DonationForm() {
  ...
  const methodPill = (active: boolean): CSSProperties => ({
  ...
  return (
    <>
      <section style={{ padding: '120px 28px 0' }}>
        <div style={wrap}>
          <Link href="/contribua" className="link-glow" style={{ fontFamily: 'var(--mono)', fontSize: 12, letterSpacing: '.06em', color: 'var(--faint)', textDecoration: 'none' }}>← Contribua</Link>
        ...
        <div data-split style={{ ...wrap, display: 'grid', gridTemplateColumns: '1.2fr .8fr', gap: 24, alignItems: 'start' }}>
          {done ? (
            <div style={{ background: 'var(--s1)', border: '1px solid var(--border)', borderRadius: 20, padding: '54px 40px', textAlign: 'center' }}>
              ...
              <button onClick={() => { setDone(false); setAmount(''); setCustom(''); }} className="btn-ghost" style={{ background: 'rgba(255,255,255,.05)', border: '1px solid var(--border)', color: 'var(--text)', fontFamily: 'var(--head)', fontWeight: 600, fontSize: 15, padding: '13px 28px', borderRadius: 99, cursor: 'pointer' }}>Fazer nova contribuição</button>
            </div>
          ) : (
            <form onSubmit={(e) => { e.preventDefault(); setDone(true); }} style={{ background: 'var(--s1)', border: '1px solid var(--border)', borderRadius: 20, padding: 36 }}>
